...
void RandomMap(u16 seed, u8 mapType, bool8 newGame){
    ...
    pathPointCenter = [int(mapWidth / 2), int(mapHeight / 2)]
    pathPoint = []
    firstDir = []
    xyAvg = [0, 0]
    points = 0
    ...
    if exitPos[1] > 0:
        ...
        points += 1
    if exitPos[2] > 0:
        pathPoint.append([exitPos[2] + int(exitSize[2] / 2), mapHeight - (MARGIN_SIZE + 1)])
        firstDir.append(1)
        xyAvg[0] -= pathPoint[points][0]
        xyAvg[1] -= pathPoint[points][1]
        points += 1
    if exitPos[3] > 0:
        pathPoint.append([mapWidth - (MARGIN_SIZE + 1), exitPos[3] + int(exitSize[3] / 2)])
        firstDir.append(0)
        xyAvg[0] -= pathPoint[points][0]
        xyAvg[1] -= pathPoint[points][1]
        points += 1
    ...
    gRandomMapData[(pathPointCenter[1] * mapWidth) + pathPointCenter[0]] = 2
    ...
    for i, obj in enumerate(entity):
        ...
        while entityPos[i] == [0,0]:
            if(rerolls > MAX_REROLLS): // Too many re-rolls, abort
                ...
                dprint(f"Re-roll entity pos:{seed} ({MARGIN_SIZE}, {(mapWidth - 1) - MARGIN_SIZE}) x ({MARGIN_SIZE}, {(mapHeight - 2) - MARGIN_SIZE}) : {xy}")
                ...
                if(check_rect(0, xy[0], xy[1] - 1, 1, 2)): // Make sure the area the entity is placed is clear, and the tile above it
                    if(check_rect(2, xy[0], xy[1] + 1, 1, 1)): // Do a separate check for the tile below the entity, alowing paths, since they are good to lead up to buildings
                        ...
                        gRandomMapData[(xy[1] * mapWidth) + xy[0]] = obj
